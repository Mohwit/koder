...
def grep_search(pattern: str, file_path: Optional[str] = None, directory: Optional[str] = None, 
                recursive: bool = False, case_sensitive: bool = True, line_numbers: bool = True,
    ...
    try:
        ...
        if file_path:
            ...
            matches = _search_in_file(abs_file_path, pattern, case_sensitive, line_numbers)
            ...
            for file_path in files_to_search:
                try:
                    matches = _search_in_file(file_path, pattern, case_sensitive, line_numbers)
                    results.extend(matches)
        ...
        raise IOError(f"Error during grep search: {str(e)}")
...
def _search_in_file(file_path: str, pattern: str, case_sensitive: bool, line_numbers: bool) -> List[str]:
...
def _get_files_to_search(directory: str, recursive: bool) -> List[str]:
    ...
    if recursive:
        ...
        for filename in os.listdir(directory):
            ...
            if os.path.isfile(file_path) and not filename.startswith('.') and _is_text_file(filename):
...
def _is_text_file(filename: str) -> bool:
    ...
    text_extensions = {
        ...
        '.md', '.txt', '.csv', '.sql', '.sh', '.bash', '.zsh', '.php', '.rb',
        '.go', '.java', '.cpp', '.c', '.h', '.hpp', '.rs', '.swift', '.kt',
        '.scala', '.clj', '.hs', '.ml', '.r', '.jl', '.pl', '.lua', '.vim',
        '.cfg', '.conf', '.ini', '.toml', '.env', '.gitignore', '.dockerignore'
    }
